package com.angular.admin.dao.cache.redis.impl;

import com.angular.admin.dao.cache.redis.RedisObjectDao;
import com.angular.admin.dao.cache.redis.ShardedJedisSentinelPool;
import com.angular.admin.domain.cache.Pair;
...
import java.util.*;
@Component
@ImportResource(locations = "classpath:spring-redis-sharded.xml")
public class RedisObjectDaoImpl implements RedisObjectDao {
    private static final Logger logger = LogManager.getLogger(RedisObjectDaoImpl.class.getName());
...
//                System.out.println("string-->"+jedis.get(key));
    ...
    public Set<String> keys(final String key) {
        ...
        return new Executor<Set<String>>(shardedJedisPool) {
            ...
            Set<String> execute() {
                Collection<Jedis> jedisC = jedis.getAllShards();
                logger.info("jedisC-->" + jedisC);
                Iterator<Jedis> iter = jedisC.iterator();
                Set<String> keys = new HashSet<String>();
                Set<String> keysSet = new HashSet<String>();
                while (iter.hasNext()) {
                    Jedis _jedis = iter.next();
                    logger.info("_jedis Host-->" + _jedis.getClient().getHost() + "_jedis Port" + _jedis.getClient().getPort());
